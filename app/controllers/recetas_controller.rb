class RecetasController < ApplicationController
  ...
  #################################################################################################
  def index
    @recetas = Receta.find(:all,:conditions => condicion_basica,:order => "nombre",:order => "nombre")
    @recetas_demas = Receta.find(:all,:conditions => "!("+condicion_basica+")",:order => "nombre")
  end
  ...
  def edit
    ...
    @ingredientes_excluidos = Ingrediente.find(:all,:conditions => condicion_basica,:order => "nombre");
  ...
  def update
    @receta = Receta.find(params[:id])
    @receta.update_attributes(params[:receta])
    redirect_to edit_receta_path(@receta)
  ...
  def anadir_ingredientes
    ...
    ingredientes=Ingrediente.find(:all,:conditions => condicion_basica,:order => "nombre")
  ...
  def modificar_ingredientes
    ...
    receta.nombre=pp[:nombre]
    receta.personas=pp[:personas]
    if receta.personas < 1 
      receta.personas=1
    end
  ...
  # Parametros
  #################################################################################################
  def guardar_etiquetaings
    
    pp=params[:etiquetaing]
    receta=Receta.find(pp[:receta].to_i)
    
    etiquetaings=Etiquetaing.find(:all,:conditions => condicion_basica)
    
    for etiquetaing in etiquetaings do
      nombre=eval("pp[:nombre_"+etiquetaing.id.to_s+"]").to_s
      if (etiquetaing.nombre != nombre )
        etiquetaing.nombre=nombre
        etiquetaing.save
      end
    end
    
    platos=Plato.find(:all,:conditions => condicion_basica)
    render(:partial => "render_tipoingredientes",:locals => {:receta => receta, :etiquetaings => etiquetaings})
  end
  ...
  def crear_receta
    ...
    pp=params[:nueva_receta]
    
    r=Receta.new
    r.usuario = usuario_actual
    r.nombre=pp[:nombre]
    r.save
    
    recetas = Receta.find(:all,:conditions => condicion_basica,:order => "nombre")
    recetas_demas = Receta.find(:all,:conditions => "!("+condicion_basica+")",:order => "nombre")

    etiquetas = Etiqueta.find(:all,:conditions => condicion_basica)
    render(:partial => "render_index",:locals => {:recetas => recetas, :recetas_demas => recetas_demas})
  ...
  # Parametros
  #################################################################################################
  def borrar_etiquetaing
    etiquetaing=Etiquetaing.find(params[:etiquetaing])
    receta=Receta.find(params[:receta].to_i)
  ...
  def crear_ingrediente
    ...
    last=Ingrediente.find(:last,:conditions => condicion_basica,:order => "created_at")
    
    ingrediente=Ingrediente.new
    ingrediente.nombre=pp[:nombre]
    ingrediente.usuario = usuario_actual
    ingrediente.tipo=(last.tipo rescue 0)
    ingrediente.save
  ...
  def editar_ingrediente
    ...
    unidadesmedida = Unidadmedida.find(:all,:conditions => condicion_basica)
    etiquetaings = Etiquetaing.find(:all,:conditions => condicion_basica)
  ...
  # Parametros
  #################################################################################################
  def cancelar_editar_etiquetaing
    receta=Receta.find(params[:receta].to_i)
  ...
  def cancelar_editar_ingrediente
    ...
    ingredientes_incluidos = Ingredientesreceta.find(:all,:conditions => "receta = "+receta.id.to_s);
    ingredientes_excluidos = Ingrediente.find(:all,:conditions => condicion_basica,:order => "nombre");
    render(:partial => "render_modificar_ingredientes",:locals => {:receta => receta,:pagina => pagina, :ingredientes_incluidos => ingredientes_incluidos, :ingredientes_excluidos => ingredientes_excluidos,:etiquetaings => etiquetaings})
  end
  ...
  # Parametros
  #################################################################################################
  def crear_tipoingrediente
    pp=params[:nuevo_tipoingrediente]
    receta=Receta.find(pp[:receta].to_i)
    ingrediente=Ingrediente.find(pp[:ingrediente])
  ...
  def convertir_unidad
    ...
    ingredientes = Ingrediente.find(:all,:conditions => condicion_basica + condicion,:order => "nombre")
  ...
  def cambiar_filtro_ingredientes
    ...
    ingredientes_excluidos = Ingrediente.find(:all,:conditions => condicion_basica + condicion_especial,:order => "nombre");
    render(:partial => "render_ingredientes_disponibles",:locals => {:receta => receta, :ingredientes_excluidos => ingredientes_excluidos})
  end
